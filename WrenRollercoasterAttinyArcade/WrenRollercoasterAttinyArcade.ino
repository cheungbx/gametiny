 ...
 *  Controls:
 *  for Tiny Joypad
 *  LEFT or RIGHT button  Makes the bird stop flying (so it sinks - enabling slides in the valleys)
 *  Fire  button Makes the bird flap (slightly) harder
 *  Also, from standby....
 *  Press and hold fire button to turn sound on and off
 *  Press and hold fire button with the left or right button held to reset high score
...
void playBird() {
  ...
  while (stopAnimate == 0) {
   ...
   if (analogRead(leftRightPin) <950) { 
     ...
     */
    // if left button pressed, Boost the speed and the height if you slide through the valleys
    if ( (playerOffset > 45) && onit &&  analogRead(leftRightPin) <950 ) {
