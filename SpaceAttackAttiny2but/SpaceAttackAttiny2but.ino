...
PB5/ADC0 - Reset if pulled low
  ...
  B1      R button
  B2      OLED SCL
  A3      up (500-750)  down(750-950) — Landscape mode
  PB3     L Button
  B4      Buzzer
  A0      right(500-750) left(750-950) - Landscape mode
  PB5     reset
...
// Interrupt handlers
ISR(PCINT0_vect){ // L button interrupt           
   if (digitalRead(butL)==LOW) fire = 1;
}

void playerIncSpaceAttack(){ // R button interrupt
  if (digitalRead(butR)==LOW) fire = 1;
}
...
*/
 // for 2 button gametiny
  DDRB   = 0b00010000;    // set PB4 "5th bit from the right" as output (for the speaker)
  PCMSK  = 0b00001000;    // pin change mask: listen to PB3 L button
...
void playSpaceAttack() {
  ...
  attachInterrupt(butR,playerIncSpaceAttack,CHANGE);
