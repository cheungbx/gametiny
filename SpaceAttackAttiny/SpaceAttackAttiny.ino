 ...
 *  Controls:
 *  for Tiny Joypad
 *  LEFT or RIGHT button  move fighter jet left or right
 *  Fire  button - fire the bullet
 *  Also, from standby....
...
ISR(PCINT0_vect){ // PB0 pin button interrupt           
   if (digitalRead(firePin)==LOW) fire = 1;
}
...
void playSpaceAttack() {
  ...
  while (stopAnimate == 0) {
    ...
    // deal with inputs
    //  fire button already hanlded by interrupt, only handle left and right.
