...
while(digitalRead(firePin) == LOW ) { // if firebutton pressed.

    nowT = millis();
    if (nowT - startT > 2000) {
      sChange = 1;     
      if (analogRead(leftRightPin) <950) {
        // if fire button and (Left or Right) button held at the same time during boot up, resdet high score 
        EEPROM.write(0,0);
        EEPROM.write(1,0);
        ssd1306_char_f6x8(8, 0, "-HIGH SCORE RESET-");  
      } 
      // if only fire button held during bott up, toggle sound on/off
      else if (mute == 0) { mute = 1; ssd1306_char_f6x8(32, 0, "-- MUTE --"); } else { mute = 0; ssd1306_char_f6x8(31, 0, "- SOUND ON -");  }    
      break;
    }
    if (sChange == 1) break;
  }  
 while(digitalRead(firePin) == LOW);// Wait till fire button released.
...
//  attachInterrupt(0,playerIncFrogger,CHANGE);
  ...
  // uncommnet if you want to use 'Up' button to jump , otherwise, Fire button for jump is already handled  in the interrupt routines ISR
   if (upDownReading> 500 && upDownReading <750 && clickLock == 0) {
       moveForward = 1;
    ...
    if (leftRightReading> 500 && leftRightReading <750 && clickLock == 0) {
        moveRight = 1;
        clickLock = 1;
        clickBase = millis();
      }

    if (leftRightReading> 750 && leftRightReading <950 && clickLock == 0) {
        moveLeft = 1;
        clickLock = 1;
        clickBase = millis();
      }
  ...
  // Handle moving left
    ...
    if (moveForward == 1 && millis() > clickBase + CLICKDELAY/2) {
      moveForward = 0;
    ...
    if (clickLock == 1 && millis() > clickBase + CLICKDELAY && leftRightReading > 950 && upDownReading > 950 && digitalRead(firePin)==HIGH) clickLock = 0; // normal debounce - reset when all keys released.
